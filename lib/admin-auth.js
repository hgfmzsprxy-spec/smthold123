...
import { bindTeamMemberAuthUser, findAdminStaffForDiscord } from "./panel-team";
...
export async function requireAdmin(request) {
  ...
  const discord = getAdminDiscordProfile(user);
  ...
  if (isAllowedAdminUser(user, token)) {
    return {
      ...
      discord,
      actor: "admin",
      isMainAdmin: isMainAdminDiscordId(discord.discordUserId),
  ...
  try {
    ...
    let member = await findAdminStaffForDiscord(discord.discordUserId, user.id, adminClient);
    if (!member) {
      return {
        error: NextResponse.json(
    ...
    if (!member.discord_auth_user_id || member.discord_auth_user_id !== user.id) {
      const bound = await bindTeamMemberAuthUser(
        {
          kind: "admin",
          memberId: member.id,
          authUserId: user.id,
          profile: { ...discord, email: user.email || null },
        },
        adminClient
      );
