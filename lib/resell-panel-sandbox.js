...
const DEFAULT_DEPOSIT_VARIANTS = [
  { slug: "deposit-20", name: "Noob", payAmount: 20, bonusPercent: 0, popular: false, sort_order: 0 },
  { slug: "deposit-50", name: "Standard", payAmount: 50, bonusPercent: 0, popular: false, sort_order: 1 },
  { slug: "deposit-100", name: "Starter", payAmount: 100, bonusPercent: 10, popular: true, sort_order: 2 },
  { slug: "deposit-250", name: "Pro", payAmount: 250, bonusPercent: 15, popular: false, sort_order: 3 },
  { slug: "deposit-1000", name: "VIP Guy", payAmount: 1000, bonusPercent: 10, popular: false, sort_order: 4 },
];
...
export function getSandboxDepositVariants() {
  return buildDepositVariants();
}
...
let liveFetch = null;
let liveDepositsPromise = null;

async function hydrateLiveDepositVariants() {
  if (!liveFetch) return;
  if (!liveDepositsPromise) {
    liveDepositsPromise = (async () => {
      try {
        const response = await liveFetch("/api/deposit-variants", { cache: "no-store" });
        const result = await response.json().catch(() => ({}));
        const variants = Array.isArray(result.variants) ? result.variants : [];
        if (variants.length && sandboxState) {
          sandboxState.depositVariants = variants;
        }
      } catch {
        // Keep sandbox fallback packages if the live catalog is unavailable.
      }
    })();
  }
  await liveDepositsPromise;
}
...
async function handleSandboxApi(url, init = {}) {
  ...
  if (path.endsWith("/bootstrap") && method === "GET") {
    await hydrateLiveDepositVariants();
...
export function installResellPanelSandboxFetch() {
  ...
  resetSandboxState();
  liveDepositsPromise = null;
  const originalFetch = window.fetch.bind(window);
  liveFetch = originalFetch;
  ...
  void hydrateLiveDepositVariants();
