...
export function formatLicenseExpiresLabel(license, now = Date.now()) {
  ...
  if (!license.expires_at && !isFrozenLicense(license) && unit === "unlimited") {
    return "Unlimited";
  }

  if (!license.expires_at && !license.activated_at && !isFrozenLicense(license)) {
    return "-";
  }

  const remainingMs = getLicenseRemainingMs(license, now);
  if (!Number.isFinite(remainingMs)) return "Unlimited";
  if (remainingMs === 0 && license.expires_at && !isFrozenLicense(license)) return "Expired";
