...
export function enrichTransactionsWithStaffProfiles(transactions, resellersOrMembers) {
  const list = Array.isArray(transactions) ? transactions : [];
  const members = [];
  (Array.isArray(resellersOrMembers) ? resellersOrMembers : []).forEach((entry) => {
    ...
    if (entry.discord_user_id || entry.id) members.push(entry);
  });
  ...
  members.forEach((member) => {
    ...
    const discordId = String(member.discord_user_id || "").trim();
    if (memberId) byMemberId.set(memberId, member);
    if (discordId) byDiscordId.set(discordId, member);
  });
  ...
  return list.map((entry) => {
    ...
    if (!hasStaff) return entry;

    const member =
      byMemberId.get(String(meta.staff_team_member_id || "").trim()) ||
      byDiscordId.get(String(meta.staff_discord_user_id || "").trim()) ||
      null;

    const discordUserId =
      meta.staff_discord_user_id || member?.discord_user_id || null;
    const avatarUrl =
      meta.staff_discord_avatar_url ||
      member?.discord_avatar_url ||
      discordFallbackAvatarUrl(discordUserId);

    return {
      ...entry,
      meta: {
        ...meta,
        staff_team_member_id: meta.staff_team_member_id || member?.id || null,
        staff_discord_user_id: discordUserId,
        staff_discord_username:
          meta.staff_discord_username || member?.discord_username || null,
        staff_discord_avatar_url: avatarUrl,
        staff_generated_at: meta.staff_generated_at || entry.created_at || null,
      },
    };
  });
}
