...
import { base64ToUint8Array } from "./base64-file";
import { getApplicationPackagePublicUrl, isApplicationPackageStorageRef } from "./application-package-storage";
...
export const LOADER_APP_IDS = {
  ...
  "kbm-aim-assist": process.env.NEXT_PUBLIC_LOADER_APP_ID_KBM_AIM_ASSIST || "c431f619947d6858",
  "permanent-spoofer": "49a4f8ea0801ead8",
  "call-of-duty": process.env.NEXT_PUBLIC_LOADER_APP_ID_CALL_OF_DUTY || "c3712051cd0d8efe",
  "apex-legends": process.env.NEXT_PUBLIC_LOADER_APP_ID_APEX_LEGENDS || "2b635880eaad98a4",
  "temporary-spoofer": process.env.NEXT_PUBLIC_LOADER_APP_ID_TEMPORARY_SPOOFER || "9eda42f5237ca930",
};
...
export function extractDiscordProfile(user) {
  ...
  const discordIdentity = identities.find((identity) => identity?.provider === "discord") || null;
  const identityData =
  ...
  const meta = { ...appMeta, ...userMeta, ...identityData };
  ...
  const idCandidates = [
    ...
    identityData.sub,
    userMeta.sub,
    // Some Discord payloads expose snowflake only under these keys.
    meta.id,
    identityData.id,
    userMeta.custom_claims?.provider_id,
  ];
  ...
  const avatarHash = String(
    meta.avatar || identityData.avatar || userMeta.avatar || meta.custom_claims?.avatar || ""
  ).trim();
  ...
  if (!avatarUrl && discordUserId && avatarHash && !/^https?:\/\//i.test(avatarHash)) {
    const ext = avatarHash.startsWith("a_") ? "gif" : "png";
    avatarUrl = `https://cdn.discordapp.com/avatars/${discordUserId}/${avatarHash}.${ext}?size=128`;
  }

  // Last resort: Discord default embed avatar from snowflake.
...
export function buildApplicationDownloadUrl(app) {
  ...
  const rawPayload = String(app?.download_file_data_base64 || "").trim();
  if (!fileName || !rawPayload) return "";

  if (isApplicationPackageStorageRef(rawPayload)) {
    return getApplicationPackagePublicUrl(rawPayload, app?.download_updated_at || "");
  }

  try {
    const bytes = base64ToUint8Array(rawPayload);
...
export function buildDownloadFileSha(app) {
  return String(app?.download_file_sha256 || "").trim();
}

export function buildLoaderDownloadAccessFromMeta(appMeta) {
  if (!appMeta) {
    return { downloadUrl: "", fileName: "", fileMeta: "No file uploaded yet.", fileSha: "" };
  }
  ...
  return {
    ...
    fileSha: buildDownloadFileSha(appMeta),
  };
}

export function getCachedLoaderDownloadAccess(appId) {
  return buildLoaderDownloadAccessFromMeta(restoreCachedApplicationMeta(appId));
}
...
export async function resolveLoaderDownloadAccess(supabaseClient, appId, profile) {
  ...
  return buildLoaderDownloadAccessFromMeta(appMeta);
}
