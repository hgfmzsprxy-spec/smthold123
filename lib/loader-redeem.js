...
function resolveLastUpdateSource(app) {
  ...
  const updatedAt = String(app?.updated_at || "").trim();
...
export function resolveLoaderDisplayMeta(app) {
  const version = String(app?.version || "").trim() || "-";
  const status =
    app?.status != null && String(app.status).trim()
      ? formatApplicationProductStatus(app.status)
      : "-";
  const lastUpdateSource = resolveLastUpdateSource(app);
  const lastUpdate = lastUpdateSource ? formatLoaderAppDate(lastUpdateSource) : "-";

  return { version, lastUpdate, status };
}

export function getStaticLoaderDisplayMetaMap(items = []) {
  return Object.fromEntries(items.map((item) => [item.slug, resolveLoaderDisplayMeta(null)]));
}
...
export function getInitialLoaderDisplayMetaMap(items = []) {
  ...
  return Object.fromEntries(
    items.map((item) => {
      ...
      return [item.slug, resolveLoaderDisplayMeta(cached)];
    }),
  );
}
...
export async function fetchLoaderDisplayMeta(supabaseClient, appId, options = {}) {
  const preferApi = options?.preferApi !== false && typeof window !== "undefined";

  if (!appId) {
    return resolveLoaderDisplayMeta(null);
  }
  ...
  if (preferApi) {
    ...
    if (apiResult.ok && apiResult.data) {
      ...
      return resolveLoaderDisplayMeta(apiResult.data);
    }

    const cached = getCachedApplicationMeta(appId);
    if (cached) {
      return resolveLoaderDisplayMeta(cached);
    }
  ...
  if (result.ok && result.data) {
    ...
    return resolveLoaderDisplayMeta(result.data);
  }
