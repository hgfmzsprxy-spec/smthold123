...
import { isFrozenLicense, getLicenseRemainingMs, licenseHasPausedTimeSnapshot } from "./license-freeze";
...
export function computeSubscriptionMetrics(license) {
  ...
  const banned = isBannedLinkedLicense(license);
  ...
  const isExpired = !frozen && !banned && remainingMs === 0 && Boolean(expiresAt);

  return {
    timeLeft: isExpired ? "Expired" : formatTimeLeft(remainingMs),
    expiryDate: banned ? "Frozen" : formatDateTime(license?.expires_at),
    progressPercent,
    statusLabel: banned ? "Banned" : frozen ? "Freezed" : isExpired ? "Expired" : "Active",
    isFrozen: frozen || banned,
    isBanned: banned,
  };
}

export function syncSubscriptionMetrics(license, mode, bannedSnapshotRef = null) {
  if ((mode !== "active" && mode !== "frozen" && mode !== "banned") || !license) {
    if (bannedSnapshotRef) bannedSnapshotRef.current = null;
    return null;
  }

  if (mode === "banned" && !licenseHasPausedTimeSnapshot(license)) {
    if (!bannedSnapshotRef?.current) {
      if (bannedSnapshotRef) {
        bannedSnapshotRef.current = computeSubscriptionMetrics(license);
      }
      return computeSubscriptionMetrics(license);
    }
    return bannedSnapshotRef.current;
  }
...
export async function fetchActiveLicensesForProfile(supabaseClient, profile) {
  if (!supabaseClient || !profile?.authUserId) return [];
  ...
  if (profile.discordUserId) {
    const { data: discordMatches, error: discordError } = await supabaseClient
      .from("licenses")
      .select(LICENSE_SELECT)
      .eq("discord_user_id", profile.discordUserId);

    if (!discordError && Array.isArray(discordMatches)) {
      rows.push(...discordMatches);
    }
  }
  ...
  for (const license of rows) {
    const key = String(license?.id || license?.license_key || "").trim();
    if (!key || seen.has(key)) continue;
    seen.add(key);

    if (isActiveLinkedLicense(license)) {
