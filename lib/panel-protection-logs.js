...
export async function readProtectionLogScreenshotsByIds(ids, admin = getSupabaseAdmin()) {
  ...
  const { data, error } = await admin.from(TABLE).select("id, screenshots").in("id", list);
  if (error) throw error;
  ...
  const entries = (data || [])
    .map((row) => {
      const slim = {
        ...row,
        screenshots: Array.isArray(row.screenshots) ? row.screenshots.map(slimScreenshotRow) : [],
      };
      return rowToEntry(slim);
    })
    .filter(Boolean);

  await attachSignedScreenshotUrls(entries, admin).catch(() => {});

  const byId = {};
  for (const entry of entries) {
    byId[entry.id] = Array.isArray(entry.screenshots) ? entry.screenshots : [];
  }
  return byId;
}
