...
export async function readProtectionLogStore(admin = getSupabaseAdmin(), options = {}) {
  ...
  // Avoid select('*') pulling unexpected large columns; screenshots are slimmed in normalize.
  ...
  const entries = sortProtectionLogEntries(
    (data || [])
      .map((row) => {
        // Drop embedded base64 before normalize so Node doesn't keep multi‑MB strings.
