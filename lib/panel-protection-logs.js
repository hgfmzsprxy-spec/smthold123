...
const MAX_PROTECTION_LOGS = 200;
const SCREENSHOT_SIGNED_URL_SECONDS = 60 * 60;

function slimScreenshotMeta(shot, { allowDataUrl = false } = {}) {
  if (!shot || typeof shot !== "object") return null;
  const path = String(shot.path || "").trim();
  const mime = String(shot.mime || "image/jpeg").trim() || "image/jpeg";
  let url = String(shot.url || "").trim();
  if (url.startsWith("data:") && !allowDataUrl) url = "";

  // Prefer storage path. Never keep base64 `data` in API payloads.
  if (path) {
    return {
      path,
      monitor: Number.isFinite(Number(shot.monitor)) ? Number(shot.monitor) : null,
  ...
  // Legacy rows stored only base64 (no storage path).
  const data = String(shot.data || "").trim();
  if (allowDataUrl && data) {
    return {
      path: "",
      monitor: Number.isFinite(Number(shot.monitor)) ? Number(shot.monitor) : null,
      width: Number.isFinite(Number(shot.width)) ? Number(shot.width) : null,
  ...
  if (url) {
    return {
      ...
      mime,
      data: "",
      url: url.startsWith("data:") && !allowDataUrl ? "" : url,
    };
  }
...
/** Persist-safe screenshot objects: path + meta only (no base64). */
function screenshotsForStorage(value) {
  return normalizeScreenshots(value)
    .map((shot) => slimScreenshotMeta(shot, { allowDataUrl: false }))
    .filter((shot) => shot && shot.path)
    .map((shot) => ({
      path: shot.path,
...
export async function readIgnoredProtectionLogUserIds(admin = getSupabaseAdmin()) {
  // Do not create buckets on every list load — that can fail and blank the feed.
  try {
...
export async function deleteProtectionLogsByFilter(
  ...
  // Never select screenshots here — rows can embed ~0.5MB base64 each.
  const { data, error: readError } = await admin
    .from(TABLE)
    .select("id, app_id, reseller_id")
  ...
  const storagePaths = [];
  const pathBatch = 5;
  for (let i = 0; i < ids.length; i += pathBatch) {
    const chunk = ids.slice(i, i + pathBatch);
    const { data: shotRows } = await admin.from(TABLE).select("id, screenshots").in("id", chunk);
    for (const row of shotRows || []) {
      for (const shot of normalizeScreenshots(row.screenshots)) {
        const path = String(shot?.path || "").trim();
...
export async function readProtectionLogStore(admin = getSupabaseAdmin(), options = {}) {
  // List payload must NEVER select `screenshots` — many rows embed ~0.5MB base64 and
  // blow Vercel memory/timeouts (500). Screenshots load lazily via entry_ids.
  void options;
  ...
  if (query.error && /column|schema cache/i.test(query.error.message || "")) {
    // Older schema fallback — still strip screenshots before returning.
    query = await admin
      .from(TABLE)
      .select("*")
      .order("created_at", { ascending: false })
  ...
  const entries = sortProtectionLogEntries(
    (data || [])
      .map((row) => {
        let entry = null;
        try {
          // Drop heavy screenshot blobs before normalize (select * fallback).
          entry = rowToEntry({ ...row, screenshots: [] });
        } catch {
          return null;
        }
        if (!entry) return null;
...
/**
 * Load slim screenshot meta (+ optional signed URLs) for a small set of log IDs.
 * Used by the admin UI for the visible page only.
 */
export async function loadProtectionLogScreenshotsByIds(
  ids,
  admin = getSupabaseAdmin(),
  options = {}
) {
  ...
  const byId = {};
  if (!unique.length) return { by_id: byId, urls: {} };

  // Small batches — each row may still hold large base64 until SQL cleanup runs.
  const batchSize = 3;
  for (let i = 0; i < unique.length; i += batchSize) {
    const chunk = unique.slice(i, i + batchSize);
    const { data, error } = await admin.from(TABLE).select("id, screenshots").in("id", chunk);
    if (error) throw error;

    for (const row of data || []) {
      const id = String(row.id || "").trim();
      if (!id) continue;
      byId[id] = normalizeScreenshots(row.screenshots)
        .map((shot) => slimScreenshotMeta(shot, { allowDataUrl }))
        .filter(Boolean);
    }
  }
  ...
  let urls = {};
  if (sign) {
    const paths = Object.values(byId)
      .flat()
      .map((shot) => String(shot?.path || "").trim())
      .filter(Boolean);
    urls = await signProtectionLogScreenshotPaths(paths, admin);
    for (const id of Object.keys(byId)) {
      byId[id] = byId[id].map((shot) => {
        if (shot?.url || !shot?.path) return shot;
        const signed = urls[shot.path];
        return signed ? { ...shot, url: signed, data: "" } : shot;
      });
    }
  }

  return { by_id: byId, urls };
}
...
export async function appendProtectionLog(entry, admin = getSupabaseAdmin()) {
  ...
  const row = {
    ...
    // Never persist base64 blobs in jsonb — storage path + meta only.
    screenshots: screenshotsForStorage(normalized.screenshots),
  };
  ...
  const { data, error } = await admin
    ...
    .insert(row)
    .select(
      "id, created_at, success, message, application, app_id, reseller_id, reseller, discord_username, discord_avatar_url, discord_user_id, discord_email, license_key, product_variant, expires_at, expiration, time_left, hwid, screenshots"
    )
    .maybeSingle();
  if (error) {
    if (/relation|does not exist|schema cache/i.test(error.message || "")) {
