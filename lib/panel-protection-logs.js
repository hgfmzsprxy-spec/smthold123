...
export async function readIgnoredProtectionLogUserIds(admin = getSupabaseAdmin()) {
  // Hot path for loaders + admin list — never block on bucket ensure / slow Storage lists.
  if (
    memoryIgnoredUsersStore?.ignored_user_ids &&
    Date.now() - memoryIgnoredUsersStore.writtenAt < 60_000
  ) {
    return memoryIgnoredUsersStore.ignored_user_ids;
  }

  try {
    // Single fixed key first (fast). Version folder listing is a fallback only.
    const legacy = await withTimeout(
      downloadIgnoredUsersPayload(admin, PROTECTION_LOG_IGNORED_USERS_PATH),
      2500,
      "ignored-users-legacy-timeout"
    );
    if (legacy) {
  ...
  try {
    const latestPath = await withTimeout(
      listLatestIgnoredUsersVersionPath(admin),
      2500,
      "ignored-users-list-timeout"
    );
    if (latestPath) {
      if (
      ...
      const versioned = await withTimeout(
        downloadIgnoredUsersPayload(admin, latestPath),
        2500,
        "ignored-users-version-timeout"
      );
      if (versioned) {
...
export async function writeIgnoredProtectionLogUserIds(userIds, admin = getSupabaseAdmin()) {
  ...
  try {
    ...
    // Still attempt the upload — bucket usually already exists in production.
    console.error("ensureProtectionsBucket:", error?.message || error);
  }
  ...
  // Trust the write immediately — verification downloads can lag on Storage CDN.
  setMemoryIgnoredUsersStore(ignored_user_ids, versionPath, true);
...
function normalizeScreenshotEntry(entry, options = {}) {
  const allowEmbeddedData = options.allowEmbeddedData === true;
  ...
  const rawData = String(entry.data || entry.b64 || entry.base64 || "").trim();
  // Never keep base64 blobs in admin list payloads — they blow past API size limits.
  const data = allowEmbeddedData ? rawData : "";
  const path = String(entry.path || entry.Path || "").trim();
  const existingUrl = String(entry.url || entry.signedUrl || entry.signed_url || "").trim();
  const httpUrl = /^https?:\/\//i.test(existingUrl) ? existingUrl : "";
  if (!data && !path && !httpUrl) {
    // Keep a placeholder so UI still shows "Monitor N" slots without shipping megabytes.
    if (entry.monitor != null || entry.Monitor != null) {
      return {
        path: "",
        monitor: Number.isFinite(Number(entry.monitor ?? entry.Monitor))
          ? Number(entry.monitor ?? entry.Monitor)
          : null,
        width: null,
        height: null,
        url: "",
  ...
  return {
    ...
    mime,
    data: allowEmbeddedData ? data : "",
    url,
  };
}

function normalizeScreenshots(value, options = {}) {
  let source = value;
  ...
  return source.map((entry) => normalizeScreenshotEntry(entry, options)).filter(Boolean);
}
...
export function normalizeProtectionLogEntry(entry) {
  ...
  return {
    ...
    screenshots: normalizeScreenshots(entry.screenshots, { allowEmbeddedData: false }),
  };
}
...
async function attachSignedScreenshotUrls(entries, admin = getSupabaseAdmin()) {
  ...
  for (const entry of list) {
    ...
    for (const shot of entry.screenshots) {
      if (shot?.url && /^https?:\/\//i.test(shot.url)) continue;
  ...
  for (const entry of list) {
    ...
    entry.screenshots = entry.screenshots.map((shot) => {
      ...
      const existingHttp = /^https?:\/\//i.test(String(shot?.url || "")) ? shot.url : "";
      const signedUrl = path ? urlMap.get(path) : "";
      return {
        path,
...
const PROTECTION_LOG_LIST_COLUMNS = [
  "id",
  "created_at",
  "success",
  "message",
...
function slimScreenshotRow(shot) {
  if (!shot || typeof shot !== "object") return shot;
  const { data: _drop, b64: _b64, base64: _base64, ...rest } = shot;
  return rest;
}

export async function readProtectionLogStore(admin = getSupabaseAdmin(), options = {}) {
  // List feed must stay tiny — never select screenshots here (base64 in jsonb times out).
  const includeScreenshots = options.includeScreenshots === true;
  const signScreenshots = includeScreenshots && options.signScreenshots !== false;
  const limit = Math.min(
    Math.max(1, Number(options.limit) || MAX_PROTECTION_LOGS),
    MAX_PROTECTION_LOGS
  );

  const selectColumns = includeScreenshots
    ? `${PROTECTION_LOG_LIST_COLUMNS}, screenshots`
    : PROTECTION_LOG_LIST_COLUMNS;
  ...
  const [resellerStore, ignoredUserIds] = await Promise.all([
  ...
  const entries = sortProtectionLogEntries(
    (data || [])
      .map((row) => {
        if (row && Array.isArray(row.screenshots)) {
          row = {
            ...row,
            screenshots: row.screenshots.map(slimScreenshotRow),
          };
        } else if (row && !includeScreenshots) {
          row = { ...row, screenshots: [] };
        }
        ...
        if (!includeScreenshots) entry.screenshots = [];
        return entry;
      })
      .filter(Boolean)
  );
  ...
  if (signScreenshots) {
    try {
      ...
      console.error("attachSignedScreenshotUrls:", error?.message || error);
    }
  }
  return { entries, ignored_user_ids: ignoredUserIds };
}

export async function readProtectionLogScreenshotsByIds(ids, admin = getSupabaseAdmin()) {
  // One/few IDs only — rows may embed multi‑MB base64 and will gateway-timeout in bulk.
  ...
  for (const id of list) {
    try {
      const { data, error } = await withTimeout(
        admin.from(TABLE).select("id, screenshots").eq("id", id).maybeSingle(),
        8_000,
        "screenshot-row-timeout"
      );
      if (error || !data) {
        byId[id] = [];
        continue;
      ...
      const mapped = shots.map((shot) => {
        const slim = slimScreenshotRow(shot) || {};
        const raw = String(shot?.data || shot?.b64 || shot?.base64 || "").trim();
        const mime = String(slim.mime || shot?.mime || "image/jpeg");
        if (!slim.url && raw) {
          // Deliver as data-URL for on-demand preview; never keep raw base64 field.
          slim.url = `data:${mime};base64,${raw}`;
        }
        slim.data = "";
        return slim;
      });

      const entry = rowToEntry({ ...data, screenshots: mapped });
      if (entry) {
        await attachSignedScreenshotUrls([entry], admin).catch(() => {});
        byId[id] = Array.isArray(entry.screenshots) ? entry.screenshots : [];
      } else {
        byId[id] = mapped;
      }
    } catch {
