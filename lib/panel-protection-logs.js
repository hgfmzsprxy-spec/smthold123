...
function slimScreenshotMeta(shot) {
  ...
  return {
    ...
    mime: String(shot.mime || "image/jpeg").trim() || "image/jpeg",
    data: "",
    // Keep only non-data URLs (signed https); drop huge data: URLs from list payloads.
...
export async function readProtectionLogStore(admin = getSupabaseAdmin(), options = {}) {
  const signScreenshots = options.signScreenshots === true;
  ...
  const entries = sortProtectionLogEntries(
    (data || [])
      .map((row) => {
        ...
        // Always slim screenshots on list reads — never ship base64 blobs to the browser.
        entry.screenshots = (Array.isArray(entry.screenshots) ? entry.screenshots : [])
          .map(slimScreenshotMeta)
          .filter(Boolean);
        return entry;
      })
      .filter(Boolean)
