...
async function attachSignedScreenshotUrls(entries, admin = getSupabaseAdmin()) {
  ...
  await Promise.all(
    list.map(async (entry) => {
      ...
      entry.screenshots = await Promise.all(
        entry.screenshots.map(async (shot) => {
          if (shot?.url) {
          ...
          try {
            const { data, error } = await admin.storage
              ...
              .createSignedUrl(shot.path, SCREENSHOT_SIGNED_URL_SECONDS);
            if (error || !data?.signedUrl) return shot;
...
function buildProductVariantLabel(license, applicationName = "") {
  const appName = String(applicationName || license?.app_name || "").trim();
  const variant =
  ...
  if (appName && variant) return `${appName} · ${variant}`;
...
export async function buildProtectionLogFromLicense({
  ...
  return normalizeProtectionLogEntry({
    ...
    product_variant: buildProductVariantLabel(license, license.app_name),
    expires_at: expiresAt,
